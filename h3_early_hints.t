...
$s->new_stream({ path => '/many-early-hints' });
$headers = get_headers($s);
is(${headers}->{':status'}, 103, '103(1) - pass many early hints');
is(${headers}->{'link'}, '<https://example.com>; rel="preconnect"',
	'header(1) - pass many early hints');
$headers = get_headers($s);
is(${headers}->{':status'}, 103, '103(2) - pass many early hints');
is(${headers}->{'link'}, '</style.css>; rel=preload; as=style',
	'header(2) - pass many early hints');
$headers = get_headers($s, all => [{ fin => 1 }]);
is(${headers}->{':status'}, 200, '200 - pass many early hints');
...
sub http_daemon {
	...
	while (my $client = $server->accept()) {
		...
		if ($uri =~ /early/) {
			...
			$links .= (CRLF . $links) x 100 if $uri =~ /huge/;
			$links .= CRLF. CRLF . 'HTTP/1.1 103 Early Hints' . CRLF
				. 'Link: </style.css>; rel=preload; as=style' if $uri =~ /many/;
			print $client <<"EOF";

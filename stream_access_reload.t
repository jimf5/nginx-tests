...
# Tests for stream access module with configuration reload.
...
my $t = Test::Nginx->new()->has(qw/stream stream_access/)
...
is(reload($t), 1, 'reload config');

is(stream('127.0.0.1:' . port(8081))->io($str, read_timeout => 0.5), '',
	'conn 2 - deny all');

is($s->io($str, read_timeout => 0.5), $str, 'conn 1 - allow all still');
